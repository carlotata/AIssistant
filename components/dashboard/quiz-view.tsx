...
export function QuizView({
   ...
   quizzes: Quiz[];
   setDeleteTarget: (
      target: { id: number; type: "conversation" | "quiz" } | null,
   ) => void;
}) {
   ...
   const [submissionResult, setSubmissionResult] = useState<FullQuiz | null>(
      null,
   );
   ...
   const randomizeTopic = async () => {
      ...
      try {
         const data = await apiFetch<{ topics: string[] }>(
            "/quizzes/topics/random",
         );
   ...
   const retakeQuiz = async (quizId: number) => {
      ...
      await ensureCsrfToken();
      try {
         const data = await apiFetch<{ quiz: FullQuiz }>(
   ...
   const submitQuiz = async () => {
      ...
      const unansweredCount =
         activeQuiz.questions.length - Object.keys(selectedAnswers).length;
      if (unansweredCount > 0) {
         setToast(
            `Please answer ${unansweredCount} more question${unansweredCount === 1 ? "" : "s"}.`,
         );
         return;
      }
               ...
               <div className="p-4 sm:p-8 rounded-2xl bg-slate-800/80 border border-indigo-500/20 shadow-lg backdrop-blur-sm space-y-5 sm:space-y-6">
                  <div className="relative flex gap-2">
                     <input
                        ...
                        placeholder="Topic..."
                        className="flex-1 bg-slate-900 p-3 sm:p-4 rounded-lg text-foreground border border-slate-700 focus:border-indigo-500 outline-none transition-all text-sm sm:text-base"
                     />
                     <button
                  ...
                  <div className="space-y-2">
                     <div className="flex items-center justify-between">
                        <label className="block text-slate-400 font-bold text-[10px] sm:text-xs uppercase">
                           Questions
                        </label>
                     ...
                     <input
                        ...
                        className="w-full h-1.5 bg-slate-700 rounded-lg appearance-none cursor-pointer accent-indigo-500 hover:accent-indigo-400 transition-all block"
                     />
                  </div>

                  {/* Difficulty Checklist */}
                  <div className="space-y-2">
                     <span className="text-slate-400 font-bold text-[10px] sm:text-xs uppercase tracking-wider">
                        Difficulty
                     </span>
                           ...
                           const colorStyles = {
                              ...
                              medium:
                                 difficulty === d
                                    ...
                                    : "bg-slate-700 border-slate-600 text-slate-400 hover:border-amber-500/50",
                              hard:
                                 difficulty === d
                                    ? "bg-rose-600 border-rose-500 text-white"
                                    : "bg-slate-700 border-slate-600 text-slate-400 hover:border-rose-500/50",
                           };
                           return (
                              <button
                                 key={d}
                  ...
                  <div className="border-t border-slate-700/50 pt-4">
                     <div className="flex flex-col gap-3">
                        <button
                           ...
                           className="w-full p-3.5 sm:p-4 rounded-lg bg-indigo-600 text-white font-bold hover:bg-indigo-500 cursor-pointer text-sm sm:text-base">
                           Create Quiz
                        </button>
                        <button
                           onClick={() => {
                           ...
                           disabled={isProcessing}
                           className="w-full p-3.5 sm:p-4 rounded-lg bg-slate-700 text-slate-300 font-bold hover:bg-slate-600 cursor-pointer text-sm sm:text-base">
                           Cancel
                        </button>
                     </div>
                  </div>
      ...
      return (
         <div className="max-w-2xl mx-auto py-4 sm:py-8 space-y-8">
            <button
               ...
               className="w-full p-6 rounded-xl bg-indigo-600 text-white font-bold text-lg hover:bg-indigo-500 transition-all shadow-lg cursor-pointer">
               Create New Quiz +
            </button>
            <div>
               ...
               <div className="space-y-3">
                  ...
                  {pending.map((q) => (
                     <div
                        key={q.id}
                        className="flex flex-col sm:flex-row sm:items-center justify-between p-5 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-foreground gap-4">
                        ...
                        <div className="flex gap-2 w-full sm:w-auto">
                           <button
                              ...
                              disabled={isProcessing}
                              className="flex-1 sm:flex-initial px-4 py-2 rounded-lg bg-indigo-600 text-white text-sm font-bold hover:bg-indigo-500 cursor-pointer">
                              Take Now
                           </button>
                           <button
                              onClick={() => deleteQuiz(q.id)}
                              disabled={isProcessing}
                              className="px-4 py-2 rounded-lg bg-red-100 dark:bg-red-900/20 text-red-600 dark:text-red-400 text-sm font-bold hover:bg-red-200 dark:hover:bg-red-900/30 cursor-pointer">
                              Delete
                           </button>
                        </div>
                     </div>
               ...
               <div className="flex flex-col gap-3 mb-4">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 w-full">
                     <input
                        ...
                        className="flex-1 w-full bg-slate-100 dark:bg-slate-800 px-4 py-2.5 rounded-lg text-sm text-foreground border border-slate-200 dark:border-slate-700 focus:ring-2 focus:ring-indigo-500/20 outline-none transition-all"
                     />
                     {/* Custom dropdown — avoids native select overlapping cards on mobile */}
                     <div className="relative w-full sm:w-40">
                        <button
                           type="button"
                           onClick={() => setDifficultyDropdownOpen((o) => !o)}
                           className="w-full flex items-center justify-between bg-slate-100 dark:bg-slate-800 px-4 py-2.5 rounded-lg text-sm text-foreground border border-slate-200 dark:border-slate-700 focus:ring-2 focus:ring-indigo-500/20 outline-none transition-all cursor-pointer">
                           <span>
                           ...
                           <svg
                              className={`h-4 w-4 text-slate-400 transition-transform duration-200 ${difficultyDropdownOpen ? "rotate-180" : ""}`}
                              fill="none"
                              stroke="currentColor"
                              viewBox="0 0 24 24">
                              <path
                        ...
                        {difficultyDropdownOpen && (
                           <>
                              ...
                              />
                              <div className="absolute left-0 right-0 top-full mt-1 z-20 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-lg overflow-hidden">
                                 {(
               ...
               <div className="space-y-4">
                  {completed
                     ...
                     <p className="text-slate-500 text-sm italic">
                        No matching completed quizzes found.
                     </p>
                  )}
                  {completed
                     .filter((q) =>
                     ...
                     .map((q) => (
                        <div
                           key={q.id}
                           className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-6 flex flex-col md:flex-row md:items-center justify-between group transition-shadow hover:shadow-sm gap-4">
                           <div className="space-y-1 flex-1 min-w-0">
                              ...
                              <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-500">
                                 <span
                                    className={`capitalize ${q.difficulty === "easy" ? "text-emerald-500" : q.difficulty === "medium" ? "text-amber-500" : "text-rose-500"}`}>
                                    {q.difficulty}
                                 </span>
                                 <span>
                                    Score:{" "}
                           ...
                           <div className="flex flex-wrap gap-2">
                              <button
                                 onClick={() => loadQuiz(q.id)}
                                 disabled={isProcessing}
                                 className="flex-1 md:flex-initial px-4 py-2 rounded-lg bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400 text-sm font-bold hover:bg-emerald-100 dark:hover:bg-emerald-900/40 transition-colors cursor-pointer">
                                 Review
                              </button>
                              <button
                                 onClick={() => retakeQuiz(q.id)}
                                 disabled={isProcessing}
                                 className="flex-1 md:flex-initial px-4 py-2 rounded-lg bg-slate-100 dark:bg-slate-800 text-foreground text-sm font-bold hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors cursor-pointer">
                                 Retake
                              </button>
                              <button
                                 onClick={() => deleteQuiz(q.id)}
                                 disabled={isProcessing}
                                 className="p-2 rounded-lg bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 text-sm font-bold hover:bg-red-100 dark:hover:bg-red-900/30 transition-colors cursor-pointer">
                                 <svg
                                    className="h-4 w-4"
                                    fill="none"
                                    stroke="currentColor"
      ...
      <div className="max-w-2xl mx-auto py-4 sm:py-8">
         ...
         <div className="flex items-center gap-3 sm:gap-4 mb-6 sm:mb-8">
            <button
               onClick={handleBack}
               disabled={isProcessing}
               className="text-slate-500 hover:text-foreground cursor-pointer shrink-0">
               ← Back
            </button>
            <h1 className="text-lg sm:text-xl font-bold text-foreground truncate">
               {quizToDisplay.quizTopic}
            </h1>
            {submissionResult && (
               <span className="ml-auto text-base sm:text-lg font-bold text-indigo-600 dark:text-indigo-400 whitespace-nowrap">
         ...
         <div className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-6 sm:p-8 shadow-lg flex flex-col min-h-87.5 sm:min-h-100">
            <div className="grow">
               ...
               <div className="space-y-3">
                  {question.options.map((option) => {
                     ...
                     let baseClasses =
                        "w-full text-left px-4 sm:px-5 py-3 rounded-lg border transition-all font-medium text-sm cursor-pointer";
                     let stateClasses =
                        "bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-foreground hover:border-indigo-400";

                     if (submissionResult) {
                        if (isCorrect) {
                           stateClasses =
                              "border-emerald-500 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-800 dark:text-emerald-100";
                        } else if (isSelected && !isCorrect) {
                           stateClasses =
                              "border-red-500 bg-red-50 dark:bg-red-900/20 text-red-800 dark:text-red-100";
                        } else {
                           stateClasses =
                     ...
                     return (
                        <button
                           ...
                           {submissionResult && isSelected && !isCorrect && (
                              <span className="font-bold shrink-0">✕</span>
                           )}
                        </button>
                     );
            ...
            <div className="mt-8 flex justify-between border-t border-slate-200 dark:border-slate-800 pt-6 sm:pt-8 gap-3 sm:gap-4">
               ...
               {submissionResult &&
               currentQuestionIdx === quizToDisplay.questions.length - 1 ? (
                  <button
                     onClick={() => {
                        setActiveQuiz(null);
                        router.replace(`${pathname}?view=quiz`);
                  ...
                  <button
                     disabled={
                        (!submissionResult &&
                           ...
                           currentQuestionIdx !==
                              quizToDisplay.questions.length - 1) ||
                        isProcessing
                     }
                     onClick={() =>
                        currentQuestionIdx ===
